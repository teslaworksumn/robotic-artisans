...
class Painter
{
	...
	public:
		...
		void setAction(string str) {action += "\n" + str; moves++;}
  ...
  if( goal == NULL) {
	...
	*/
	...
	while (Goal_Test(Current_Painter) && (cycle_counter < 100000))
	{
	   	...
	   	if(Unexplored_Painters.size() > 10)
	   	{
			Unexplored_Painters.erase(Unexplored_Painters.begin(),Unexplored_Painters.end() - 10);
		}
		//cout << "unexplored size: " << Unexplored_Painters.size()<< "explored size: " << Explored_Painters.size() << endl;
	   	...
	   	for(i = 0; i < (int)Unexplored_Painters.size(); i++)
	   	{
	   	...
	   	if(Current_Painter.Color() == 0)
	    {
	   		...
	   		string_stream << "-5"<<" "<<Child_Painter.XPos()<<" "<<Child_Painter.YPos() <<" "<<Current_Painter.Color()<<" "<<Child_Painter.Color();
					...
					if(Current_Painter.XPos()-1 != 0)
					{
						...
						if(Current_Painter.Color() == goal[new_index])
						{
		    				if(Current_Painter.Color() != Current_Painter.Canvas(new_index))
		    				{
								...
								string_stream <<"-1"<<" "<<Child_Painter.XPos()<<" "<<Child_Painter.YPos();
								Child_Painter.setAction(string_stream.str());
								string_stream.str("");	//clear the string stream
								//cout << Child_Painter.getAction() << endl;
								Child_Painter.setParent(Explored_Painter_counter);
	    		...
	    		for(i=0; i<Current_Painter.getSize(); i++)
	    		{
	    			...
	    			if(Current_Painter.Color() == goal[i])
	    			{
	    				...
	    				if(Current_Painter.Color() != Current_Painter.Canvas(i))
	    				{
							...
							string_stream << "-1"<<" "<< Child_Painter.XPos() << " " << Child_Painter.YPos() <<endl<<"-3";
					   		Child_Painter.setAction(string_stream.str());
					   		//cout << Child_Painter.getAction() << endl;
					   		string_stream.str("");	//clear the string stream
					   		Child_Painter.setParent(Explored_Painter_counter);
	...
	cout << Current_Painter.getAction() << endl;
	/*
	cout << endl << "solution found with " << cycle_counter << " Painters expanded" << endl << "Final State" << endl;
	Current_Painter.Print();
	cout << "Solution action cost: " << Current_Painter.Cost() << "\nMoves: " << Current_Painter.Moves() << endl << endl;
	*/

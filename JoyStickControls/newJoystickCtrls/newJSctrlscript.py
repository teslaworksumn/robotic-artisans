			...
			if type & 0x01:
				...
				if button:
					...
					if value:
						...
						print "%s released" % (button)
					if value and button == 'trigger':
						ser.write('X0.0 0.0 0.0 1.0\n')
				...
				if axis:
					...
					if(axis == 'rz'):
						'''if(Angles['x']<=minx):
							...
							Angles['x'] = maxx-1'''
						ser.write('X'+str(-5*fvalue)+' 0.0 0.0 0.0\n')
					if(axis == 'y'):
						'''if(Angles['y']<=miny):
							Angles['y'] = miny+1
						elif(Angles['y']>=maxy):
							Angles['y'] = maxy-1'''
						ser.write('X0.0 '+str(10*fvalue)+' 0.0 0.0\n')
					if(axis == 'hat0y'):
						'''if(Angles['rz']<=minz):
							Angles['rz'] = minz+1
						elif(Angles['rz']>=maxz):
							Angles['rz'] = maxz-1'''
						ser.write('X 0.0 0.0 '+str(5*fvalue)+' 0.0\n')
					#if(axisCount%2==0):        
